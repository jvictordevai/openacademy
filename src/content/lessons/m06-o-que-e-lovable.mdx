...
task:
  prompt: "Liste 3 telas internas que você pagaria pra existir hoje na sua empresa — coisa que hoje vive em Excel + email + reunião. Para cada: input (de onde vem o dado) + output (o que mostra) + quem usa."
  criteria: "3 telas listadas. Cada uma com input descrito (planilha X, sistema Y), output (lista, métrica, fluxo), e usuário (quem abre toda semana)."
  minutes: 5
---

## TL;DR

Lovable é uma plataforma onde você **descreve em português o app que precisa** — e ela gera o app inteiro: frontend, backend, banco de dados, autenticação, conectores. Em 20-30 minutos você sai do zero pra um app rodando numa URL — sem programador.
...
Aplique agora — siga o passo a passo prático: [Construindo dashboard real no Lovable](/licoes/m06-build-dashboard).
